...
#             nn.Conv2d(3, 16, 4, 2, 1),  # Input: 3x800x800, Output: 16x400x400
#             nn.ReLU(True),
#             nn.Conv2d(16, 32, 3, 1, 1),  # Output: 32x400x400
#             nn.ReLU(True),
#             nn.MaxPool2d(2, 2),  # Output: 32x200x200
#             nn.Conv2d(32, 64, 4, 2, 1),  # Output: 64x100x100
#             nn.ReLU(True),
#             nn.Conv2d(64, 128, 3, 1, 1),  # Output: 128x100x100
#             nn.ReLU(True),
#             nn.MaxPool2d(2, 2),  # Output: 128x50x50
#             nn.Conv2d(128, 256, 4, 2, 1),  # Output: 256x25x25
#             nn.ReLU(True)
#         )
#         # Latent space
#         self.fc_mu = nn.Linear(256 * 25 * 25, 256)  # Output: 256
#         self.fc_logvar = nn.Linear(256 * 25 * 25, 256)  # Output: 256
#
#         # Decoder
#         self.decoder_fc = nn.Linear(256, 256 * 25 * 25)  # Input: 256, Output: Flattened 256x25x25
#
#         self.decoder = nn.Sequential(
#             nn.ReLU(True),
#             nn.ConvTranspose2d(256, 128, 4, 2, 1),  # Input: 256x25x25, Output: 128x50x50
#             nn.ReLU(True),
#             nn.ConvTranspose2d(128, 64, 4, 2, 1),  # Output: 64x100x100
#             nn.ReLU(True),
#             nn.ConvTranspose2d(64, 32, 4, 2, 1),  # Output: 32x200x200
#             nn.ReLU(True),
#             nn.ConvTranspose2d(32, 16, 4, 2, 1),  # Output: 16x400x400
#             nn.ReLU(True),
#             nn.ConvTranspose2d(16, 3, 4, 2, 1),  # Output: 3x800x800
#             nn.Sigmoid()
#         )
...
#         # Decoder
#         z = self.decoder_fc(z)             # Map z to the initial shape for the decoder
#         z = z.view(z.size(0), 256, 25, 25)  # Reshape z to the expected input dimensions of the decoder
#         x_recon = self.decoder(z)          # Pass z through the decoder to get the reconstruction
#
#         return x_recon, mu, logvar
